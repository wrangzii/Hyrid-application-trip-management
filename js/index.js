...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #necessaryStuff').text($('#page-create #frm-register #necessaryStuff').val());
    $('#page-create #frm-confirm #weatherCondition').text($('#page-create #frm-register #weatherCondition').val());
...
function registerTrip(e) {
    ...
    let necessaryStuff = $('#page-create #frm-register #necessaryStuff').val();
    let weatherCondition = $('#page-create #frm-register #weatherCondition').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description, Risk, Date, necessaryStuff, weatherCondition) VALUES (?, ?, ?, ?, julianday('${date}'), ?, ?)`;
        tx.executeSql(query, [name, destination, description, risk, necessaryStuff, weatherCondition], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #necessaryStuff').text(result.rows[0].NecessaryStuff);
                $('#page-detail #detail #weatherCondition').text(result.rows[0].WeatherCondition);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #necessaryStuff`).val(result.rows[0].necessaryStuff);
                $(`#page-detail #frm-update #weatherCondition`).val(result.rows[0].weatherCondition);
...
function updateTrip(e) {
    ...
    let necessaryStuff = $('#page-detail #frm-update #necessaryStuff').val();
    let weatherCondition = $('#page-detail #frm-update #weatherCondition').val();

    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Risk = ?, Date = julianday('${date}'), necessaryStuff = ?, weatherCondition = ? WHERE Id = ?`;

        tx.executeSql(query, [name, destination, description, risk, necessaryStuff, weatherCondition, id], transactionSuccess, transactionError);
